...
from widgets.items import LibraryDelegate
from widgets.items import DirectoryDelegate
...
def Make (Base):
    ...
    class Intermediate (Base):
        ...
        def __init__(self, parent=None):
            self.resizing = False
            super(Intermediate, self).__init__(parent=parent)
        ...
        def createFolder (self, index, name):
            ...
            if not name or os.path.exists(newPath):
                updateItem.setData(dict(type="plusfolder"), QtCore.Qt.EditRole)
        ...
        def getDirItems (self, path, filterFavorites=False, showTypes=None):
            if showTypes is None:
                showTypes = ["foldercolors", "usdasset", "usdmaterial"]
            if not path:
                return []
            library = []
            self.assetsNames = []
            ...
            for name in os.listdir(path):
                ...
                if dataType in ["usdasset", "usdmaterial"]:
                    if dataType not in showTypes:
                        continue
                    with Metadata.MetadataManager(folderPath, update=False) as metadata:
                        data = metadata
        ...
        def drawBrowserItems (self, path, filterFavorites=False):
            if not path:
                ...
                browserItems = self.getDirItems(path, filterFavorites=filterFavorites )
            hasLibrary = False
            hasFolder = False
            hasAsset = False
            hasMaterial = False
            ...
            if hasFolder or not hasLibrary and not hasAsset and not hasMaterial and not filterFavorites:
        ...
        def sortItems (self, library):
            ...
            for data in library:
                ...
                if dataType == "labellibrary":
                    ...
                    colors += [data]
            for data in [libraries, folders, usdasset, usdmaterial, guides]:
            ...
            for data in [colors]:
                data.sort(key=code)
            library = (labelL + labelF + labelA + labelM + plus + libraries + folders
                       + usdasset + usdfile + usdmaterial + guides + colors)
            return library
        ...
        def changeEvent (self, event):
            super(Intermediate, self).changeEvent(event)
            if self.resizing:
                with Settings.Manager(self.theme.app, True) as settings:
        ...
        def resizeMath (self, value):
            minimunWidth = Settings.UIGlobals.Options.minimumWidth
            maximunWidth = Settings.UIGlobals.Options.maximumWidth
            margin = Settings.UIGlobals.Options.margin
            optionWidth = self.width() - value - margin*2
            space = self.width() - self.Browser.minimumWidth() - optionWidth - margin*2
            if maximunWidth >= optionWidth >= minimunWidth and space > 0:
                return optionWidth

        def resizeEvent (self, event):
            super(Intermediate, self).resizeEvent(event)
            self.resizing = True
