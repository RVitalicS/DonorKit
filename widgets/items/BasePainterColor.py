...
import toolkit.core.colorspace as colorspace
    ...
    def decorated (function):
        def wrapped (self):
            ...
            fillArea = QtCore.QRect(
                ...
                self.width, self.previewHeight + self.space)
            if fill:
                self.previewColor = QtGui.QColor(self.data.get("color"))
            else:
                self.previewColor = QtGui.QColor(self.theme.color.colorSpace )
...
def infoguide (function):
    def wrapped (self):
        ...
        countLabelArea = QtCore.QRect(
            self.infoArea.x(), self.infoArea.y() + offsetRaw,
            self.infoArea.width(), self.infoArea.height() - offsetRaw)
...
def infocolor (function):
    def wrapped (self):
        ...
        redLabelArea = QtCore.QRect(
            self.infoArea.x(), self.infoArea.y() + offsetBlock,
            self.infoArea.width(), self.infoArea.height() - offsetBlock)
        self.painter.setPen( QtGui.QColor(self.theme.color.colorText) )
        self.painter.drawText(
...
def favorite (function):
    def wrapped (self):
        ...
        if self.hover or self.controlMode:
            ...
            if self.favorite:
                image = graphics.recolor(image, hexStar)
            else:
