...
class Painter (object):
    ...
    def copyPreviewCrop (self, rect):
        offsetY = self.previewHeight - self.previewImage.height()
        crop = QtCore.QRect(
            rect.x() - self.space, rect.y() - self.space - offsetY,
    ...
    def getOverlayHex (self, rect):
        ...
        if not self.previewImage.isNull():
            lightness = graphics.lightnessAverage(
                self.copyPreviewCrop(rect) )
            if not lightness:
                lightness = QtGui.QColor(self.theme.color.iconSpace ).valueF()
        else:
...
class Editor (QtWidgets.QWidget):
    ...
    def paintEvent (self, event):
        ...
        self.Item.paint(painter, option, self.Item.index)
        painter.end()
    
    def mouseMoveEvent (self, event):
        self.Item.pointer = QtCore.QPoint(event.x(), event.y())
        self.update()
...
class Delegate (QtWidgets.QStyledItemDelegate):
    ...
    def leaveAction (self):
        self.setModelData(
            self.sender(), self.parent().model(),
            self.sender().Item.index )
        self.closeEditor.emit(
            self.sender(), QtWidgets.QAbstractItemDelegate.NoHint)
