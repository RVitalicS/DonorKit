...
from widgets import AssetExportUI
...
class Dialog (BaseExport.Make(Base)):
    ...
    def interpretTags (self, choice):
        ...
        path = ostree.getPathUSD(assetpath, assetname)
        if path:
            animationPart = naming.getAnimationName(assetname)
            animationName = self.ExportOptions.animationOptions.animationNameCombobox.getName()
            if animationPart == animationName != "":
                highlightAnimation = True
            variantPart = naming.getVariantName(assetname)
    ...
    def versionChoice (self, text):
        if text:
            ...
            animationList = naming.getAnimationList(path, version)
    ...
    def getAssetName (self, final=False):
        ...
        return naming.createAssetName(
            ...
            animation=animation, final=final )
